...
class TranslationRuleSet:
    ...
    def standard(cls) -> TranslationRuleSet:
        ...
        """
        rs = cls()
        _data_path = Path(__file__).parent / "data" / "translation_rules.json"
        with open(_data_path, "r", encoding="utf-8") as _f:
            _raw_list = json.load(_f)
        for _r in _raw_list:
            rs.add(TranslationRule(
                rule_id=_r["rule_id"],
                from_lang=_r["from_lang"],
                to_lang=_r["to_lang"],
                source_pattern=_r["source_pattern"],
                target_pattern=_r["target_pattern"],
                transform_kind=TransformKind(_r["transform_kind"]),
                confidence_factor=_r["confidence_factor"],
                notes=_r.get("notes", ""),
            ))
